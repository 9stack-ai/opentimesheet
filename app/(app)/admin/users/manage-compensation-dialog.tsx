...
import { addCompensation, deleteCompensation } from "./compensation-actions";
                ...
                user.compensations.map((c) => (
                  <tr key={c.id} className="border-t">
                    ...
                    <td className="px-2 py-1.5 text-right text-muted-foreground">
                      {bpsToPercent(c.taxWithholdingRateBps)}%
                    </td>
                    <td className="px-2 py-1.5 text-right">
                      <form action={deleteCompensation}>
        ...
        <form action={addCompensation} className="flex flex-col gap-3 rounded-md border p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="grid gap-1">
              <Label>Loại</Label>
              <select name="kind" defaultValue="HOURLY" className={selectClass}>
            ...
            <div className="grid gap-1">
              ...
              <Input name="fixedMonthlySalary" type="number" min={0} defaultValue={0} />
            </div>
            <div className="grid gap-1">
              <Label>Từ ngày</Label>
            ...
            <div className="grid gap-1">
              ...
              <Input name="costRate" type="number" min={0} defaultValue={0} />
            </div>
            <div className="grid gap-1">
              <Label>Đơn giá bán (VND/giờ)</Label>
              <Input name="billableRate" type="number" min={0} defaultValue={0} />
            </div>
            <div className="grid gap-1">
              <Label>% thuế TNCN</Label>
              <Input name="taxWithholdingPercent" type="number" min={0} max={100} step="0.01" defaultValue={0} />
            </div>
            <div className="grid gap-1">
              <Label>% BH công ty</Label>
              <Input name="employerCostPercent" type="number" min={0} max={100} step="0.01" defaultValue={0} />
            </div>
          </div>
